...
public interface GitLogRepository extends CrudRepository<GitLogEntity, GitLogPrimaryKey> {
    ...
    @Query(value = "select * from git_log where ordinal <= (select ordinal from git_log where git_sha = :currentSha and artifact_id = :artifactId) and ordinal > (select ordinal from git_log where git_sha = :previousSha and artifact_id = :artifactId)", nativeQuery = true)
    ...
    @Query(value = "select * from git_log where ordinal <= (select ordinal from git_log where git_sha = :sha and artifact_id = :artifactId)", nativeQuery = true)
